...
/**
 * Check if user agent is mobile or tablet browser.
 * @returns {boolean} true | false
 */
const checkNonDesktopBrowsers = () => {
 ...
 * @returns {(string|null)} Version of safari browser.
 */
const getSafariVersion = () => {
  const userAgent = navigator.userAgent || navigator.vendor || window.opera;
  if (userAgent)
    return (
      /safari/i.test(userAgent) && userAgent.match(/version\/(\d+[.]?\d+)/i)[1]
    );
  return null;
};

export { toSlug, checkNonDesktopBrowsers, getSafariVersion };
